...
import { useAdmins } from "@/app/lib/firestore/admins/read";
import { deleteAdmin } from "@/app/lib/firestore/admins/write";
...
const ListView = () => {
  ...
  return (
    <div className="flex flex-1 flex-col gap-3 rounded-xl md:px-0 md:pr-5">
      <h1 className="text-xl">Lista de Admins</h1>
      ...
      <tr key={index + 1}>
        ...
        <td className="border-y bg-white px-3 py-2">
          <div className="flex flex-col">
            <h2> {item?.name}</h2>
            <h3 className="text-xs text-gray-600">{item?.email}</h3>
          </div>
        </td>
